...
final void setoffset(int n, int offset) {
	if (n >= MAXOFFSETS)
		return;
	int shift = 8 * (7 - n);
	offsets &= (~(0xFFL << shift));
 ...
 */
public Name(DNSInput dnsin) throws WireParseException {
	int len, pos;
	boolean done = false;
	byte [] label = new byte[MAXLABEL + 1];
	...
	while (!done) {
		...
		case LABEL_NORMAL:
			...
			if (len == 0) {
				...
				label[0] = (byte)len;
				dnsin.readByteArray(label, 1, len);
				append(label, 0, 1);
			}
			break;
		case LABEL_COMPRESSION:
			...
			if (Options.check("verbosecompression"))
				System.err.println("currently " + dnsin.current() +
						   ", pointer to " + pos);

			if (pos >= dnsin.current() - 2)
...
public Name fromDNAME(DNAMERecord dname) throws NameTooLongException {
	...
	for (int i = 0; i < MAXOFFSETS && i < plabels + dlabels; i++) {
