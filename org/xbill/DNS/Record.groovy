...
{
	Record rec;
	rec = getEmptyRecord(name, type, dclass, ttl, dnsin != null);
	if (dnsin != null) {
		if (dnsin.remaining() < length)
			throw new WireParseException("truncated record");
		dnsin.setActive(length);

		rec.rrFromWire(dnsin);
...
static Record fromWire(DNSInput dnsin, int section, boolean isUpdate) throws IOException {
	int type, dclass;
	long ttl;
	int length;
	Name name;
...
static Record fromWire(DNSInput dnsin, int section) throws IOException {
	return fromWire(dnsin, section, false);
}
