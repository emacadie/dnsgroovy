 ...
 * TCP.
 */
    byte [] generateReply(Message query, byte [] b_in, int length, Socket s) throws IOException {
      Header header;
      boolean badversion;
      int maxLength;
      int flags = 0;
	...
	if (queryTSIG != null) {
	  ...
	  if (tsig == null || tsig.verify(query, b_in, length, null) != Rcode.NOERROR) { 
	    return formerrMessage(b_in);
	  }
	}
  ...
  public byte [] formerrMessage(byte [] b_in) {
    Header header;
    try {
      header = new Header(b_in);
    } 	catch (IOException e) {
      return null;
    }
  ...
  public void TCPclient(Socket s) {
    try {
      ...
      b_in = new byte[inLength];
      dataIn.readFully(b_in);

      Message query;
      byte [] response = null;
      try {
	query = new Message(b_in);
	response = generateReply(query, b_in, b_in.length, s);
	if (response == null) { 
	  return;
	}
      } catch (IOException e) {
	response = formerrMessage(b_in);
      }
      dataOut = new DataOutputStream(s.getOutputStream());
  ...
  public void serveUDP(InetAddress addr, int port) {
    try {
      ...
      byte [] b_in = new byte[udpLength];
      DatagramPacket indp = new DatagramPacket(b_in, b_in.length);
      DatagramPacket outdp = null;
      while (true) {
	indp.setLength(b_in.length);
	try {
	  sock.receive(indp);
	...
	try {
	  query = new Message(b_in);
	  response = generateReply(query, b_in, indp.getLength(), null);
	  if (response == null) { 
	    continue;
	  }
	} catch (IOException e) {
